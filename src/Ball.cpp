...
bool Ball::checkCollisionWithPlayer(const Player& p)
{
...
int Ball::update(const Player& p1, const Player& p2, float delta)
{
  ...
  if (resetCooldown) {
    resetTimer -= delta;
    if (resetTimer <= 0.0f) resetCooldown = false;
    else return 0;
  }
  x += speedX * delta;
  y += speedY * delta;
...
void Ball::bounce(const Player& p)
{
  ...
  speedY = -B_SPEED * speedMultiplier * sin(bounceAngle);
  speedX = (speedX > 0 ? 1 : -1) * B_SPEED * speedMultiplier * cos(bounceAngle);
}

void Ball::draw() const
{
  DrawRectangle((int)(x - radius), (int)(y - radius), 2 * radius, 2 * radius, WHITE);
}
